...
- Account/profile hub (`/profile`) separated from planning (`/plan`); напомняния за плана: една секция с канали (`push_enabled`, имейл prefs) + глобално време за всички запазени фестивали (`/api/plan/reminders` с `applyToAllSaved`).
