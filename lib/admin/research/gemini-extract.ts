...
ЗАГЛАВИЕ (title):
- title = КРАТКОТО собствено име на фестивала, така както се появява (напр. „Фолклорна плетеница", НЕ „Седмият музикален и танцов фестивал „Фолклорна плетеница"").
- Придържай се към формулировката от заявката (user_query), когато текстът се отнася за същото събитие — НЕ пренаписвай и НЕ разширявай с описателни/разказни добавки (пореден номер като „седми", „издание", тип като „музикален и танцов", име на града), освен ако те не са част от самото собствено име.
- Запази годината само ако тя присъства в заявката; иначе не добавяй година.
