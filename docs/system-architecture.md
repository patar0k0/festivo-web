...
- **API (session + service role after authorization):** `POST /api/organizer/organizers` (create profile + active owner), `POST /api/organizer/claims` (pending owner claim; blocked when an active owner already exists), `GET /api/organizer/memberships`, `POST /api/organizer/pending-festivals`, `PATCH /api/organizer/pending-festivals/[id]` (only `submission_source=organizer_portal` + active membership + `status=pending`).
