...
- `/organizer` (entry) adapts CTAs by session and `organizer_members`: dashboard and portal tools only after at least one **active** membership (`owner`/`admin`/`editor`). Pending-only members see a review message without dashboard access. `/organizer/dashboard`, `/organizer/submissions`, `/organizer/festivals/new`, and submission edit require active membership server-side; `/organizer/profile/new` (creates organizer + active owner via API) and `/organizer/claim` (pending membership; admin approves at `/admin/organizer-claims`) stay available for onboarding. Bulgarian UI; separate from `/admin`.
