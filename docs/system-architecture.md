...
**Configuration:** `GEMINI_API_KEY` (or `GOOGLE_AI_API_KEY`); optional `GEMINI_RESEARCH_MODEL` (default `gemini-3.5-flash`; auto-falls back to `gemini-3.1-flash-lite` on 429/quota — 500 RPD free tier), `GEMINI_RESEARCH_TIMEOUT_MS`.
