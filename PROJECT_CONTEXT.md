...
- **РџСѓР±Р»РёС‡РЅР° РґРµС‚Р°Р№Р»РЅР° СЃС‚СЂР°РЅРёС†Р°:** РјРµРґРёСЏ РіР°Р»РµСЂРёСЏ С‡СЂРµР· `components/festival/FestivalGallery.tsx` РІ `FestivalDetailClient`; РґРѕРїСЉР»РЅРёС‚РµР»РЅРё СЃРЅРёРјРєРё Рё РѕРїС†РёРѕРЅР°Р»РЅРѕ РІРёРґРµРѕ (YouTube/Facebook) СЃРµ РїР°Р·СЏС‚ РІ `festival_media` (`type` image/video, `sort_order` Р·Р° СЂРµРґ), Р±РµР· РґР° РїСЂРµРјР°С…РІР°С‚ `festivals.hero_image`. Pending: `gallery_image_urls` + `video_url` РґРѕ РѕРґРѕР±СЂСЏРІР°РЅРµ, РїСЂРё approve -> `festival_media`. SQL: `scripts/sql/20260330_festival_media_gallery_video.sql`.
...
- Transactional email queue (`email_jobs`): enqueue-first към Postgres, batch send през `GET /api/jobs/email` (jobs secret / Vercel cron) и Resend (`lib/email/*`); регистър на типове + BG шаблони в `emails/*` (Phase 2); enqueue след успех от `POST /api/organizer/claims`, `POST /api/organizer/pending-festivals`, `POST /admin/api/organizer-members/[id]/approve|reject`, `POST /admin/api/pending-festivals/[id]/approve|reject`; **напомняния за запазени фестивали** като втори канал към същите `notification_jobs` reminder слотове (`GET /api/notifications/run` → `reminder-1-day-before` / `reminder-same-day`, виж `docs/notification-system.md`); опционално `EMAIL_ADMIN` за админ алерти; dev `GET /api/test-email` с `type` + JSON `payload` за преглед на шаблони; `dedupe_key` се задава при enqueue (`lib/email/emailDedupeKeys.ts`); claim approve/reject изпраща имейл само при реален `pending`→transition в DB
