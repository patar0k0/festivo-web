...
- **Admin users (`/admin/users`):** optional shadow row in `public.users` holds `deleted_at`, optional `deleted_by` / `deleted_reason` on soft-delete; middleware sends deleted accounts to `/account-deleted` and auth-banned users to `/banned` (distinct from soft-delete). Soft-delete clears refresh tokens and updates `users` in one RPC (`admin_set_user_soft_deleted`); hard auth delete is followed by `admin_sweep_user_after_auth_delete`. App roles are a single row per user in `user_roles` (RLS: users may `select` only their row). APIs under `/admin/api/users/*` support restore (ensures `user_roles` row with `user` when missing), forced session invalidation (`admin_invalidate_auth_sessions`), password recovery link generation, bulk actions (max 50 ids, `{ success, failed }`), and **development-only** hard delete; organizer memberships can be revoked or re-rolled (including `viewer`) via `/admin/api/organizer-members/[id]`.
