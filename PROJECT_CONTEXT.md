...
- **Admin users (`/admin/users`):** optional shadow row in `public.users` holds `deleted_at` for soft-delete (login blocked in `middleware.ts`); app roles are a single row per user in `user_roles` (`user` / `organizer` / `admin` / `super_admin`, with `is_admin()` treating `admin` and `super_admin` as staff). APIs under `/admin/api/users/*` support restore, forced session invalidation (`admin_invalidate_auth_sessions`), password recovery link generation, bulk ban/soft-delete/role assign, and **development-only** hard delete; organizer memberships can be revoked or re-rolled (including `viewer`) via `/admin/api/organizer-members/[id]`.
