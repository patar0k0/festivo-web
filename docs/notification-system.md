...
**Категории имейли (Phase 5 — preference layer):** типовете са класифицирани в `lib/email/emailTypeCategory.ts`; решението „да се изпрати ли към потребител“ за опционални типове е в `canSendEmailTypeToUser` (`lib/email/emailPreferences.ts`) **след** надеждно зареждане на prefs. **Задължителни transactional** (`organizer-claim-*`, `festival-submission-received`, `festival-approved`/`rejected`, `test`) — **не** се спират от `user_email_preferences` или unsubscribe; при грешка при четене на prefs изпращането **не** се блокира (fail-open спрямо prefs lookup). **Админ алерти** (`admin-new-claim`, `admin-new-submission`) — към `EMAIL_ADMIN`, без user preference gating; същото fail-open при prefs lookup (не се прилагат към тези типове). **Опционални напомняния** (`reminder-1-day-before`, `reminder-same-day`) и **бъдещи optional категории** (`optional_marketing`, …) — при **явен opt-out** (`reminder_emails_enabled=false`, `unsubscribed_all_optional`, и т.н.) → без enqueue/send; при **недостъпни prefs / грешка при lookup** → без enqueue/send (**fail-closed**), ясни логове; на `email_jobs` процесор: `last_error=preference_blocked` (изрично изключено) или `preference_lookup_failed` (неуспешен lookup), `attempts=max_attempts`, без смислен retry. Unsubscribe / reminder prefs **не** могат да се заобиколят чрез временна DB грешка. **Бъдещи marketing/organizer optional** — полетата `marketing_emails_enabled` / `organizer_update_emails_enabled` са foundation; няма отделни marketing кампании в тази фаза.

**Напомняния по имейл (без втори scheduler):** планирането остава в `notification_jobs` (`scheduleSavedFestivalReminders` / `syncReminderJobsForPreference`). **Push vs имейл (Phase 5):** за `job_type=reminder`, `push_enabled=false` **вече не** отменя целия job: първо се опитва enqueue на reminder имейл (ако `user_email_preferences` позволява). Ако имейлът е enqueue-нат, job-ът се маркира `sent` **без** FCM. Ако push е изключен и имейл не може да се изпрати (prefs / няма Auth имейл / грешка), job → `cancelled`, `last_error=reminder_no_active_channel`. Ако push е включен, но няма `device_tokens`, а имейлът е enqueue-нат — job → `sent` (имейл-only). За **не-reminder** job types, `push_enabled=false` остава `cancelled` (`push_disabled`). **Тихи часове** за `reminder`: job се отменя (`quiet_hours_skip`) — **без** имейл и без push (без промяна). **Имейл prefs:** при успешно заредени prefs — ако `reminder_emails_enabled=false` или `unsubscribed_all_optional=true` → пропуск на enqueue + лог „preference disabled“; при **неуспешен batch/single load** на prefs → пропуск на enqueue (fail-closed) + лог „preference lookup failed“. При `push_enabled=false` и без имейл: `notification_jobs.last_error` е `reminder_email_preference_disabled`, `reminder_email_preference_lookup_failed` или `reminder_no_active_channel` (други причини), за да се различава от opt-out. При невалиден FCM payload → `failed` (`invalid_payload`) — **без** имейл. Enqueue на reminder имейл е **преди** `device_tokens`. Дедупликация: `lib/email/emailDedupeKeys.ts`. При **изпращане** от процесора (`lib/email/processEmailJobs.ts`), опционалните редове се проверяват отново; при изричен opt-out → `preference_blocked`; при lookup грешка → `preference_lookup_failed`; без retry. **Unsubscribe:** глобален `unsubscribe_token` на потребител; страница `GET /unsubscribe/[token]`, `POST /api/email/unsubscribe` (service role). **Профил:** секция „Напомняния“ в `/profile` — `GET/POST /api/email/preferences` (имейл), `GET/POST /api/notification-settings` (`push_enabled` и **`default_plan_reminder_type`**: по подразбиране за *нови* запазвания в плана), `GET /api/plan/reminders` (обобщение за текущо запазените) и по избор `POST /api/plan/reminders` с `{ applyToAllSaved: true, reminderType }` (уеднаквяване на вече запазените + `syncReminderJobsForPreference` по фестивал). При `POST /api/plan/festivals` се прилага `default_plan_reminder_type` към новия запис в `user_plan_reminders` и се синхронизират jobs. **Footer:** опционални линкове в reminder шаблона (`EmailFooter` + `FestivalReminderEmail`). SQL: `scripts/sql/20260405_user_email_preferences.sql`; default timing: `scripts/sql/20260406_user_notification_default_plan_reminder_type.sql`. Извън scope: marketing кампании, богат preference center, per-festival email granularity.
...
- **Оперативен преглед (admin UI):** `/admin/email-jobs` — списък с филтри по статус, `delivery_status`, тип, група (напомняния / организаторски / админ алерти) и търсене; обобщаващи броячи (pending, failed, bounced, delivered, изпратени последните 24 ч). Детайл `/admin/email-jobs/[id]` — обобщение на реда, маскиран payload, timeline от `email_events`. Това е **оперативна видимост за дебъг**, не аналитичен дашборд; извън scope: resend/retry бутони, CSV, маркетинг отчети, тежка аналитика.
...
| `followed_organizer` | normal | След успешно одобряване на pending (`POST .../approve`) | Мачва `festival_organizers` + fallback `festivals.organizer_id` към `user_followed_organizers`; уважава `notify_followed_organizers` + `push_enabled`; dedupe по `user_id+festival_id`. |
| `trending` | normal | `GET /api/cron/worker` в `sat_09` слот (weekly) | Една седмична препоръка на потребител: лек score (saves + freshness + proximity + promoted/trending flags), регионален boost по следвани градове, и preference gate `notify_trending_alerts`. |
...
- За останалите: макс. **2** изпращания на потребител за 24 ч; макс. **1** „промо“ (`weekend`, `new_city` или `trending`) комбинирано за 24 ч.
...
- Тихи часове: по подразбиране `22:00 -> 09:00` (локално време), когато няма explicit `quiet_hours_*` в `user_notification_settings`. `reminder` (saved festivals) е high-priority и не се quiet-skip-ва; `weekend` / `new_city` / `followed_organizer` / `trending` се препланират на следващ момент извън quiet window (стъпка 15 мин).
...
Типове в payload: например `festival_reminder`, `festival_updated`, `weekend_nearby`, `new_festival_in_city`, `followed_organizer_new_festival`, `trending_festival`.

## Дедупликация

- Уникален ключ: `notification_jobs.dedupe_key` (upsert с `ignoreDuplicates`).
- Допълнително: time-window дедупликация + cooldown по тип (виж таблицата по-горе). Практически лимити: trending ≤ 1 / 48h, nearby (`new_city`) ≤ 1 / 24h, organizer (`followed_organizer`) ≤ 1 / 12h per organizer (`payload_json.scope_key=organizer:<id>`).
...
- `user_notification_settings` — `push_enabled`, `only_saved`, `quiet_hours_*`, `notify_nearby_discovery`, `notify_trending_alerts`.
...
- `push_delivery_audit` — durable send/open audit за push (`notification_type`, `user_id`, `device_token`, payload summary, `deep_link`, send status, provider response, `opened_at`, `created_at`), плюс source за mobile inbox и admin diagnostics (`scripts/sql/20260508_push_delivery_audit_table.sql`).
