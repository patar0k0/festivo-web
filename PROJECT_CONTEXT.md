...
- Transactional email queue (`email_jobs`): enqueue-first към Postgres, batch send през `GET /api/jobs/email` (jobs secret / Vercel cron) и Resend (`lib/email/*`); при успех се пази Resend id в `provider_message_id`; липсващ ключ и непознат `type` минават през retry/fail с ясни `last_error`; dev-only `GET /api/test-email` enqueue-ва тестов job (не direct send), отговор `outcome` при dedupe
