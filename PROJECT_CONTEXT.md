...
- **Admin users (`/admin/users`):** optional shadow row in `public.users` holds `deleted_at`, optional `deleted_by` / `deleted_reason` on soft-delete (reason sanitized server-side); middleware sends deleted accounts to `/account-deleted` and auth-banned users to `/banned` (distinct from soft-delete). Soft-delete clears refresh tokens and updates `users` in one RPC (`admin_set_user_soft_deleted`, with client retries on transient failure); hard auth delete is followed by `admin_sweep_user_after_auth_delete` (jsonb counts, retries + logs). App roles are a single row per user in `user_roles` (RLS: users may `select` only their row). APIs under `/admin/api/users/*` support restore (auth must exist, not actively banned; `user_roles` inserted before clearing `deleted_at`), forced session invalidation (`admin_invalidate_auth_sessions`), password recovery link generation, bulk actions (max 50 ids, per-id guards for self / last staff admin / sole organizer owner on soft-delete and ban; stricter Upstash limits on destructive user routes), **development-only** hard delete (idempotent if auth already gone), and `logUserSecurityAudit` / `admin_audit_logs` for security actions; organizer memberships can be revoked or re-rolled (including `viewer`) via `/admin/api/organizer-members/[id]`.
