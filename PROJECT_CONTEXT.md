...
- Transactional email queue (`email_jobs`): enqueue-first към Postgres, batch send през `GET /api/jobs/email` (jobs secret / Vercel cron) и Resend (`lib/email/*`); регистър на типове + BG шаблони в `emails/*` (Phase 2); enqueue след успех от `POST /api/organizer/claims`, `POST /api/organizer/pending-festivals`, `POST /admin/api/organizer-members/[id]/approve|reject`, `POST /admin/api/pending-festivals/[id]/approve|reject`; опционално `EMAIL_ADMIN` за админ алерти; dev `GET /api/test-email` с `type` + JSON `payload` за преглед на шаблони; `dedupe_key` се задава при enqueue (`lib/email/emailDedupeKeys.ts`); claim approve/reject изпраща имейл само при реален `pending`→transition в DB
