...
- **Jobs bypass** (applies to `/api/jobs/*`, `/api/notifications/*`, and `/api/cron/*`): Vercel Cron header `x-vercel-cron`, or `x-job-secret` matching `JOBS_SECRET`. Same bypass is used for the origin check below.
