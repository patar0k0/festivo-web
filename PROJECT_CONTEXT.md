...
- **Research → pending geocoding:** `POST /admin/api/research-festival/create-pending` normalizes BG location text and attempts geocoding (`Google Geocoding` first, `OSM Nominatim` fallback, BG country bias). Results are stored on pending rows (`latitude`, `longitude`, `place_id`, `geocode_provider`) and copied to `festivals` on approve. Admin fallback endpoint `POST /api/admin/geocode` supports manual “Намери координати” in pending/published edit forms.
