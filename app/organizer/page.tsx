...
const ctaDark =
  "inline-flex items-center justify-center rounded-xl bg-[#0c0e14] px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-black";

const ctaOutline =
  "inline-flex items-center justify-center rounded-xl border border-black/[0.14] bg-white px-5 py-2.5 text-sm font-semibold text-[#0c0e14] transition hover:bg-neutral-50";
      ...
      <div className="mx-auto max-w-lg space-y-6">
        <div className="rounded-2xl border border-emerald-200/45 bg-gradient-to-br from-emerald-50/50 via-white/95 to-amber-50/40 p-6 shadow-sm ring-1 ring-emerald-100/25 md:p-8">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            {!loggedIn ? (
              <>
                <Link href="/login?next=/organizer" className={ctaDark}>
                  Вход
                </Link>
                <Link href="/signup?next=/organizer" className={ctaOutline}>
                  Регистрация
                </Link>
              </>
            ) : hasActive ? (
              <>
                <Link href="/organizer/dashboard" className={ctaDark}>
                  Към таблото
                </Link>
