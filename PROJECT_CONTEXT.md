...
`Research with AI` (`/api/admin/research-ai`, `lib/research/perplexity.ts`) uses **Perplexity only for URL discovery** (JSON `urls` plus API `citations`), then **fetches each ranked page**, extracts **schema.org JSON-LD** (name, dates, location) and **Bulgarian text dates** (e.g. „2 май“, dotted numeric dates), and **merges** sources (earliest start date, modal city, longest title, combined descriptions). **Confidence** uses existing heuristics, then may **bump** when 2+ pages agree on start date or city. The API result includes optional **`research_report`** (per-URL log and reasoning) for admin review.
