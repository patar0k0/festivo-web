...
- Admin audit logging (`admin_audit_logs`) records successful admin write actions (best-effort, non-blocking) for moderation and catalog mutations; optional **`dedupe_key`** prevents duplicate rows on retry; read-only review at `/admin/activity` (paginated filters, service-role read after admin session check)
- **Admin users (`/admin/users`):** optional shadow row in `public.users` holds `deleted_at`, optional `deleted_by` / `deleted_reason`, **`banned_until`** (mirror of Auth on admin ban/unban via `admin_sync_user_banned_until`), **`ban_sync_error`** when Auth and DB diverge after a failed ban rollback, and **`cleanup_pending`** during hard-delete / self-delete paths. Middleware uses a **few-second** TTL cache for `deleted_at` / `banned_until` (invalidated on admin ban / soft delete / restore / hard delete / sweep cleanup) and treats **JWT or DB** ban as active. `user_sweep_retry_queue` ( **`next_retry_at`**, **`attempts`**, **`seen_in_auth_before`**, lease via **`locked_until`**; claim RPC includes expired leases with **`FOR UPDATE SKIP LOCKED`**) survives auth row removal and drives **`GET /api/jobs/user-sweep-retry`** when `admin_sweep_user_after_auth_delete` needs backoff retries with jitter (`5m/15m/1h/6h` + random 0-30%); successful sweeps clear **`cleanup_pending`**. The sweep cron uses a global `cron_locks` guard and also performs lightweight retries for `ban_sync_error` users, clearing the flag on successful re-sync. Soft-delete clears refresh tokens and updates `users` in one RPC (`admin_set_user_soft_deleted`, with client retries on transient failure); hard auth delete is followed by sweep RPC (validates non-zero deletes when auth existed). Restore checks organizer membership integrity (valid organizer FK, role/status enums). Bulk destructive actions use per-operation Upstash limits and **sorted** ids for deterministic per-user outcomes. `logAdminAction` retries once async on failure and writes second-level **`dedupe_key`** to avoid duplicate rows on retry without suppressing legitimate repeated actions. App roles are a single row per user in `user_roles` (RLS: users may `select` only their row). APIs under `/admin/api/users/*` support restore, forced session invalidation, password recovery, bulk actions (max 50 ids, guards for self / last staff admin / sole organizer owner), **development-only** hard delete, and security audit logging; organizer memberships via `/admin/api/organizer-members/[id]`.
