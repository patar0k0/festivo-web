...
// FK hint required on `organizers` embed: festival_organizers is M2M, so the embed without FK name is ambiguous.
export const FESTIVAL_SELECT_MIN =
  "id,title,slug,city_id,city_slug,start_date,end_date,start_time,end_time,occurrence_dates,category,hero_image,image_url,is_free,status,is_verified,promotion_status,promotion_started_at,promotion_expires_at,promotion_rank,lat,lng,place_id,description,ticket_url,price_range,festival_media(url,type,sort_order,is_hero),cities:cities!festivals_city_id_fkey(name_bg,slug,is_village),organizer:organizers!festivals_organizer_id_fkey!left(id,name,slug,plan,plan_started_at,plan_expires_at,organizer_rank)";
...
export const FESTIVAL_SELECT_ORGANIZER_PROFILE =
  "id,title,slug,city_id,city_slug,start_date,end_date,start_time,end_time,occurrence_dates,category,hero_image,image_url,is_free,status,promotion_status,promotion_started_at,promotion_expires_at,promotion_rank,lat,lng,place_id,description,ticket_url,price_range,festival_media(url,type,sort_order,is_hero),cities:cities!festivals_city_id_fkey(name_bg,slug,is_village),organizer:organizers!festivals_organizer_id_fkey!left(id,name,slug)";
