...
- `SMART_RESEARCH_IMAGE_RERANK` (optional, server-only; set to `0` to disable the Gemini vision image reranker that scores/reorders candidate festival covers by relevance — default on)
...
- `FB_PAGE_ID` (server-only; numeric Facebook Page id на Festivo страницата — за "Публикувай във Facebook" бутона в admin festival edit; споделен с festivo-workers)
- `FB_PAGE_ACCESS_TOKEN` (server-only; long-lived/System-User Page access token с `pages_manage_posts` за Graph API постване)
- `FB_GRAPH_VERSION` (optional; override на Graph API версията, подразбиране `v21.0`)
