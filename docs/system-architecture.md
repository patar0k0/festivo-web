...
- **Public festival detail UX:** hero emphasizes a single primary action („Напомни ми“); calendar export is secondary in the same row. The rail groups „Добави в моя план“, navigation, and reminder timing without duplicating the hero CTA. Quick facts render as a compact strip when data exists. **`/profile` — напомняния:** една секция „Напомняния“ обединява `push_enabled` (`POST /api/notification-settings`), имейл напомняния (`POST /api/email/preferences`) и време за всички запазени фестивали (`GET` + `POST /api/plan/reminders` с `applyToAllSaved`, вътре `syncReminderJobsForPreference` по фестивал); unsubscribe линк от reminder имейли → `/unsubscribe/[token]`.
